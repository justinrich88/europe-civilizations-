...
- **It is not actually monotonic, as shipped.** `room` hits the floor at 87.5% full (`1 − units/cap = FLOOR` when `units/cap = 0.875`) and holds flat from there to capacity — but growth is `units × room`, and `units` keeps rising while `room` sits still, so growth itself *rises* over that last eighth: 43.75% of peak at 87.5% full climbing to 50.0% of peak at capacity, a ~14% increase. (Peak is `units × room` at half-full, `0.25 × cap`; at capacity it's `1.0 × cap × 0.125 = 0.125 × cap`, i.e. 50% of peak — not the minimum, as the "monotonic" framing implied.)

  The floor itself is intentional — it's what stops a station wobbling asymptotically at 99.7% of capacity forever (see the comment near `sim/growth.js:260`). The non-monotonic bump this produces in the last eighth looks like an unintended side effect of that fix, not a chosen part of the shape, and is flagged here as an open question for the balance pass rather than fixed — whether to accept it, retune the floor, or reshape the tail is a balance decision. **The design's conclusion still holds regardless:** a half-full city still grows twice as fast as a full one (peak `0.25 × cap` vs `0.125 × cap` at capacity), so "full cities should be spent" is intact — what was wrong was the stated *shape* of the curve near the top, not the incentive it creates.
...
- **Proportion** — each source sends a share of the units currently sitting in it. A **persistent** 25 / 50 / 75 / All setting applies to every volley, chosen with `1 2 3 4`, and it **stays where the player put it** until the player moves it again. It opens at 25%, a tuning constant (`BAL.SEND_FRACTION_DEFAULT`) — that is a starting position, not a resting one.

  *Revised twice, 2026-07, both times on the player's instruction.* First the persistent 75% and its commit-time modifiers `⇧` (all) / `⌥` (half) were replaced by the digits `1-4` — same job, one hand, no timing, and `⇧` was already additive-select on the same pointer. Then the amount was made one-shot, relaxing to 25% after every volley on the argument that a sticky All turns one absent-minded click into an emptied city.

  **That second change was wrong and is reverted.** The amount is chosen for a *situation*, not for a click: a player who has decided to commit hard has decided it about an exchange, and re-picking the digit before each volley taxes the plan rather than the mistake. It also made the bar lie — it displayed a number that the next click would not use, which is known-issue #18 exactly. The safety it was buying is already bought better by the hover preview, which shows the literal payload of every source before the click lands. `⌘` survives untouched, because keeping the group is not an amount.
